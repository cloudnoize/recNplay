...
func saveWav(ab *AudioBuffer, sr uint32, filePath string) {
	var wh wavreader.WavHHeader
	var buff [44]byte
	//CHunkID
	copy(buff[:], []byte("RIFF"))
	//ChunkSize
	audioBytes := len(ab.q.Q) * 2        //16bits e.g. 2 bytes
...
func GetFileName() string {
	return "tmp" + time.Now().String() + ".wav"
}
